...
public:
    ...
    Stigmergy_Grid(double Lx, double Ly, double s_SG, double w, double p_min, double p_max, double gam_s, double c_max)
    {
        d1 = round(Ly / s_SG);
        d2 = round(Lx / s_SG);
        P_min = p_min;
    ...
    void index_particle_to_grid(int x_i, int y_i, double theta_i, double l_i, double w, double s_SG, double Lx, double Ly)
    {
        ...
        for (int i = 0; i<=(l_seg); i++)
        {
            ...
            F_seg_x = -gamma_s * diff_x_s ;
            F_seg_y = -gamma_s * diff_y_s ;
    ...
    void decay(double beta_p, double beta_s, double dt)
    {
        for (int i = 0; i < d1; i++)
        {
            for (int j = 0; j < d2; j++)
            { 
		if (this -> Stig_Grid_P_next[i][j] != 0) 
	           {
		    this->Stig_Grid_P_next[i][j] -= this->Stig_Grid_P_next[i][j] * beta_p * dt;
		    }
			

		if (this -> Stig_Grid_S_next[i][j] != 0) 
                   {
    ...
    void add_counts(double k_p, double k_s, double dt, double c_max)
    {
