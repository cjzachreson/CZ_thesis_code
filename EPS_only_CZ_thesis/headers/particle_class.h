...
public:
    ...
    Particle(double, double, double, double, double, double, double, double, double, int, int, int, int, double, double);
    ...
    void Twitch(const double, const double, const double, const double, const Stigmergy_Grid &, const double, const BINS &, vector<Particle> &, const double, double);
    
    void move_and_reset(double dt, int N, double Lx, double Ly)
    {
        this->X += vx * dt;
        this->Y += vy * dt;
    ...
    void stig_index(double s_SG, double Lx, double Ly)
    {
        this->xi_sg = index_X_to_grid(s_SG, Lx, this->X);
        this->yi_sg = index_Y_to_grid(s_SG, Ly, this->Y);
    }
...
Particle::Particle(double x_i, double y_i, double l_i, double phi_i, double theta_i,
                   ...
                   int N, int xi_max, int yi_max, double Lx, double Ly)
{
    XI_max = xi_max;
    YI_max = yi_max;
    this->X = x_i;
...
void Particle::Twitch(const double dt, const double Lx, const double Ly, const double s_SG, const Stigmergy_Grid & s_g, const double s_CL, const BINS & bins, vector<Particle> & p, const double P_attch_particle, double T_rev)
{
    retraction_timer -= dt;
    reversal_timer -= dt;
    ...
    if (retraction_timer <= 0)
    {
        ...
        pilus_length = rng()  * (pilus_range - w/2) + w/2 ; 

        // factor of w/2 prevents numerical artifact from rounding of cell length to nearest w/2 during stigmergy grid indexing, artifact only appears if Tret is very fast and pili can bind just in front of the cell pole
        ...
        {
            int xi_sg_ap = index_X_to_grid(s_SG, Lx, attachment_point_x);
            int yi_sg_ap = index_Y_to_grid(s_SG, Ly, attachment_point_y);
            
            this->P_attch = s_g.find_P_attch(yi_sg_ap, xi_sg_ap, s_SG);
