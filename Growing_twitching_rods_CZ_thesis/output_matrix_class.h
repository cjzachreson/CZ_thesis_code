...
/* This class creates an output matrix containing particle coordinates, orientations, and lengths 
*/
...
public:
    ...
    // to accommodate cell division
    void more_particles(int N_new)
    {
        for (int i = 0; i < d1; i++)
        {
            this -> XYTL[i].resize(N_new);
            
            for (int j = d2; j < N_new; j++)
            {
                this->XYTL[i][j].resize(d3);
            }
        }
